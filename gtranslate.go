...
func Translate(text string, from language.Tag, to language.Tag, googleHost ...string) (string, error) {
	...
	translated, err := translate(text, from.String(), to.String(), false, 2, 0, http.Client{})
	if err != nil {
		return "", err
	}
...
func TranslateWithParams(text string, params TranslationParams) (string, error) {
	...
	translated, err := translate(text, params.From, params.To, true, params.Tries, params.Delay, params.HttpClient)
	if err != nil {
		return "", err
	}
	return translated, nil
